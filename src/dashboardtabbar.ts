 ...
 */
export
class DashboardTabBar<T> extends Widget {
  /**
   * Construct a new tab bar.
   *
   * @param options - The options for initializing the tab bar.
   */
  constructor(options: DashboardTabBar.IOptions<T> = {}) {
    ...
    this.renderer = options.renderer || DashboardTabBar.defaultRenderer;
   ...
   */
  get currentChanged(): ISignal<this, DashboardTabBar.ICurrentChangedArgs<T>> {
   ...
   */
  get tabMoved(): ISignal<this, DashboardTabBar.ITabMovedArgs<T>> {
   ...
   */
  get tabActivateRequested(): ISignal<this, DashboardTabBar.ITabActivateRequestedArgs<T>> {
   ...
   *
   */
  get tabAddRequested(): ISignal<this, DashboardTabBar.ITabAddRequestedArgs> {
   ...
   */
  get tabCloseRequested(): ISignal<this, DashboardTabBar.ITabCloseRequestedArgs<T>> {
   ...
   */
  get tabDetachRequested(): ISignal<this, DashboardTabBar.ITabDetachRequestedArgs<T>> {
   ...
   */
  readonly renderer: DashboardTabBar.IRenderer<T>;
   ...
   */
  insertBehavior: DashboardTabBar.InsertBehavior;

  /**
   * The selection behavior when removing a tab.
   */
  removeBehavior: DashboardTabBar.RemoveBehavior;
   ...
   */
  get orientation(): DashboardTabBar.Orientation {
   ...
   * This controls whether the tabs are arranged in a row or column.
   */
  set orientation(value: DashboardTabBar.Orientation) {
  ...
  protected onUpdateRequest(msg: Message): void {
    ...
    //if (titles.length > 0) {
    //  content[0] = h.div({ className: 'lm-TabBar-button', title: 'Lock' }, 'lock')
    //}
    ...
    if ((!this._dragData || !this._dragData.dragActive) && this.title.closable) {
      content[titles.length] = h.div({ className: 'lm-TabBar-button', title: 'New Dashboard' }, 'add')
    }
  ...
  private _evtMouseDown(event: MouseEvent): void {
    ...
    // or the add button
    if (event.button === 1 || (this.title.closable && index === (tabs.length - 1))) {
      return;
    }
  ...
  private _evtMouseMove(event: MouseEvent): void {
    ...
    if (this.title.closable && data.index === tabs.length - 1) {
      return;
    }
  ...
  private _evtMouseUp(event: MouseEvent): void {
    ...
    if (!data.dragActive) {
      ...
      if (this.title.closable && index === tabs.length - 1) {
  ...
  private _orientation: DashboardTabBar.Orientation;
  private _previousTitle: Title<T> | null = null;
  private _dragData: Private.IDragData | null = null;
  private _tabMoved = new Signal<this, DashboardTabBar.ITabMovedArgs<T>>(this);
  private _currentChanged = new Signal<this, DashboardTabBar.ICurrentChangedArgs<T>>(this);
  private _tabCloseRequested = new Signal<this, DashboardTabBar.ITabCloseRequestedArgs<T>>(this);
  private _tabAddRequested = new Signal<this, DashboardTabBar.ITabAddRequestedArgs>(this);
  private _tabDetachRequested = new Signal<this, DashboardTabBar.ITabDetachRequestedArgs<T>>(this);
  private _tabActivateRequested = new Signal<this, DashboardTabBar.ITabActivateRequestedArgs<T>>(this);
}


/**
 * The namespace for the `DashboardTabBar` class statics.
 */
export
namespace DashboardTabBar {
  /**
  ...
  interface IOptions<T> {
     ...
     */
    orientation?: DashboardTabBar.Orientation;
  ...
  class Renderer implements IRenderer<any> {
    ...
    renderLabel(data: IRenderData<any>): VirtualElement {
      let unset = (!data.title.closable || data.title.label !== '');
      let content = unset ? data.title.label : 'Dbl-click to edit';
...
namespace Private {
   ...
   */
  export
  function snapTabLayout(tabs: HTMLCollection, orientation: DashboardTabBar.Orientation): ITabLayout[] {
   ...
   */
  export
  function layoutTabs(tabs: HTMLCollection, data: IDragData, event: MouseEvent, orientation: DashboardTabBar.Orientation): void {
   ...
   */
  export
  function finalizeTabPosition(data: IDragData, orientation: DashboardTabBar.Orientation): void {
   ...
   */
  export
  function resetTabPositions(tabs: HTMLCollection, orientation: DashboardTabBar.Orientation): void {
    each(tabs, tab => {
      if (orientation === 'horizontal') {
